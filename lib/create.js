...
function onCreate(req, res, next) {
  ...
  if (!qs.category) return res.send(400, 'Missing distribution category name!');
  ...
  utils.create(category, utils.time.week(), utils.time.weeks(2), utils.time.weeksAgo(1))
    .then(function(delta) {
      res.send(200, 'Distribution was successfully created!');
    })
    .catch(function(e) {
      res.send(400, 'There was an error creating the distribution!');
    });
}
