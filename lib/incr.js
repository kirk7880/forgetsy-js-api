...
function onIncr(req, res, next) {
  ...
  var incrementDate = qs.dateValue && qs.dateType ? utils.time[qs.dateType](qs.dateValue) : new Date().getTime();
  ...
  for (var i=0; i<len; i++) {
    ...
    utils.create(category, utils.time.week(), utils.time.weeks(2), utils.time.weeksAgo(1), function(e, delta) {
      if (delta) {
        delta.incr({
          bin: bin
          ,by: incrementBy
